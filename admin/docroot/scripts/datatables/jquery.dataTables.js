...
(/** @lends <global> */function( $ ) {
	...
	{
		var cell_start_time = new Date().getTime();
		...
		if ( sData === undefined )
		{
			...
			console.log('  [ ' + iRow + '-' + iCol + ' ] GetCellData [' + sSpecific + ' ] ' + oCol.sDefaultContent + ' default content (' + ((new Date().getTime()) - cell_start_time) + ') SECONDS');
		...
		else if ( typeof sData === 'function' )
		{
			...
			console.log('  [ ' + iRow + '-' + iCol + ' ] GetCellData [' + sSpecific + '] function (' + ((new Date().getTime()) - cell_start_time) + ') SECONDS');
			return sData();
		}
	
		if ( sData === null && sSpecific == 'display' )
		{
			console.log('  [ ' + iRow + '-' + iCol + ' ] GetCellData [' + sSpecific + '] display (' + ((new Date().getTime()) - cell_start_time) + ') SECONDS');
			return '';
		}
		console.log('  [ ' + iRow + '-' + iCol + ' ] GetCellData [' + sSpecific + '] final (' + ((new Date().getTime()) - cell_start_time) + ') SECONDS');
		return sData;
	}
	...
	{
		var row_start_time = new Date().getTime();
		...
		if ( row.nTr === null )
		{
			...
			for ( i=0, iLen=oSettings.aoColumns.length ; i<iLen ; i++ )
			{
				...
				var columnname = oCol.sName;
				nTd = nTrIn ? anTds[i] : document.createElement( oCol.sCellType );
				cells.push( nTd );
				console.log('  [ ' + iRow + '-' + i + ' ] Cell created ' + columnname + ' (' + ((new Date().getTime()) - start_time) + ') SECONDS');
				// Need to create the HTML if new, or if a rendering function is defined
				if ( !nTrIn || oCol.mRender || oCol.mData !== i )
				{
					var start_time = new Date().getTime();
					console.log('  [ ' + iRow + '-' + i + ' ] Col Data rendering for ' + columnname + ' ');
					nTd.innerHTML = _fnGetCellData( oSettings, iRow, i, 'display' );
					var end_time = new Date().getTime();
					console.log('  [ ' + iRow + '-' + i + ' ] Col Data rendered for ' + columnname + ' (' + (end_time - start_time) + ') SECONDS');
				}
				...
				if ( oCol.fnCreatedCell )
				{
					...
					console.log('  [ ' + iRow + '-' + i + ' ] Col cellCreated rendered for ' + columnname + ' (' + ((new Date().getTime()) - start_time) + ') SECONDS');
				}
			}
		...
		console.log('  [ ' + iRow + ' ] ROW CREATED IN (' + ((new Date().getTime()) - row_start_time) + ') SECONDS');
	}
	...
	{
		var start_time = new Date().getTime();
		console.log('start drawing (' + ((new Date().getTime()) - start_time) + ')');
		...
		console.log('init display (' + ((new Date().getTime()) - start_time) + ')');
		...
		console.log('defer load check (' + ((new Date().getTime()) - start_time) + ')');
	
		if ( aiDisplay.length !== 0 )
		{
			var iStart = bServerSide ? 0 : iDisplayStart;
			var iEnd = bServerSide ? oSettings.aoData.length : iDisplayEnd;
			console.log('append rows ' + iStart + ' to ' + iEnd + ' (' + ((new Date().getTime()) - start_time) + ')');
			for ( var j=iStart ; j<iEnd ; j++ )
			{
				var iDataIndex = aiDisplay[j];
				var aoData = oSettings.aoData[ iDataIndex ];
				console.log('row create tr start ' + iRowCount + ' (' + ((new Date().getTime()) - start_time) + ')');
				if ( aoData.nTr === null )
				{
					_fnCreateTr( oSettings, iDataIndex );
				}
				console.log('row create tr end ' + iRowCount + ' (' + ((new Date().getTime()) - start_time) + ')');
	
				var nRow = aoData.nTr;
	
				console.log('row stripe start ' + iRowCount + ' (' + ((new Date().getTime()) - start_time) + ')');
				...
				console.log('row stripe end ' + iRowCount + ' (' + ((new Date().getTime()) - start_time) + ')');
	
				console.log('row callback start ' + iRowCount + ' (' + ((new Date().getTime()) - start_time) + ')');
				/* Row callback functions - might want to manipulate the row */
				_fnCallbackFire( oSettings, 'aoRowCallback', null,
					[nRow, aoData._aData, iRowCount, j] );
				console.log('row callback end ' + iRowCount + ' (' + ((new Date().getTime()) - start_time) + ')');
				anRows.push( nRow );
				iRowCount++;
				console.log('row push end ' + iRowCount + ' (' + ((new Date().getTime()) - start_time) + ')');
			}
		...
		console.log('append rows (' + ((new Date().getTime()) - start_time) + ')');
	
		/* Header and footer callbacks */
		_fnCallbackFire( oSettings, 'aoHeaderCallback', 'header', [ $(oSettings.nTHead).children('tr')[0],
			_fnGetDataMaster( oSettings ), iDisplayStart, iDisplayEnd, aiDisplay ] );
		console.log('header callback (' + ((new Date().getTime()) - start_time) + ')');
	
		_fnCallbackFire( oSettings, 'aoFooterCallback', 'footer', [ $(oSettings.nTFoot).children('tr')[0],
			_fnGetDataMaster( oSettings ), iDisplayStart, iDisplayEnd, aiDisplay ] );
	
		console.log('footer callback (' + ((new Date().getTime()) - start_time) + ')');
		
		var body = $(oSettings.nTBody);
	
		body.children().detach();
		body.append( $(anRows) );
		console.log('body element append (' + ((new Date().getTime()) - start_time) + ')');
		
		/* Call all required callback functions for the end of a draw */
		_fnCallbackFire( oSettings, 'aoDrawCallback', 'draw', [oSettings] );
		console.log('draw callback fire (' + ((new Date().getTime()) - start_time) + ')');
	
		/* Draw is complete, sorting and filtering must be as well */
		oSettings.bSorted = false;
		oSettings.bFiltered = false;
		oSettings.bDrawing = false;
		console.log('end drawing (' + ((new Date().getTime()) - start_time) + ')');
	}
