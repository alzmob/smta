...
(function($) {
    ...
    function SlickGrid($container,data,columns,options) {
        ...
        function init() {
            ...
            $headerScroller = $("<div class='slick-header panel-footer text-muted' style='overflow:hidden;position:relative;padding:0px;' />").appendTo($container);
            $headers = $("<div class='slick-header-columns' style='width:100000px; left:-10000px' />").appendTo($headerScroller);

            //$secondaryHeaderScroller = $("<div class='slick-header-secondary' style='overflow:hidden;position:relative;' />").appendTo($container);
            ...
            $viewport = $("<div class='slick-viewport' tabIndex='0' hideFocus style='width:100%;outline:0;position:relative;'>").appendTo($container);
            ...
            $container.parent().bind("resize", resizeAndRender);
        ...
        function getMaxSupportedCssHeight() {
            ...
            var testUpTo = 5000000; //($.browser.mozilla) ? 5000000 : 1000000000;  
        ...
        function createColumnHeaders() {
            ...
            for (i = 0; i < columns.length; i++) {
                ...
                var header = $("<div class='panel-heading slick-header-column' id='" + uid + m.id + "' />")
        ...
        function setupColumnReorder() {
            $headers.sortable({
                ...
                helper: "clone",
                placeholder: "slick-sortable-placeholder panel-heading slick-header-column",
        ...
        function measureCellPaddingAndBorder() {
            var tmp = $("<div class='panel-heading slick-header-column' style='visibility:hidden'>-</div>").appendTo($headers);
        ...
        function autosizeColumns() {
            ...
            for (i = 0; i < columns.length; i++) {
                c = columns[i];
                widths.push(parseInt(c.width));
                existingTotal += parseInt(c.width);
                shrinkLeeway += parseInt(c.width) - Math.max(c.minWidth || 0, absoluteColumnMinWidth);
            }
        ...
        function appendRowHtml(stringArray,row) {
            ...
            stringArray.push("<div class=' " + css + "' row='" + row + "' style='top:" + (options.rowHeight*row-offset) + "px'>");
            ...
            for (var i=0, cols=columns.length; i<cols; i++) {
				...
				if ( m.editor )
					cellCss += ' editable-cell';

                stringArray.push("<div class='" + cellCss + "'>");

                // if there is a corresponding row (if not, this is the Add New row or this data hasn't been loaded yet)
                if (d) {
                	if (m.field.indexOf(".") == -1) {
                		stringArray.push(getFormatter(m)(row, i, d[m.field], m, d));
                	} else {
                		var nested_value_function = function(obj, field) {
                			path=field.split('.');
                			var res=obj;
                			for (var i=0;i<path.length;i++) { res=res[path[i]]; }
                			return res;
                		};
                		stringArray.push(getFormatter(m)(row, i, nested_value_function(d, m.field), m, d));
                	}
                }
        ...
        function resizeCanvas() {
			...
			if (options.autoHeight) { // use computed height to set both canvas _and_ divMainScroller, effectively hiding scroll bars.
				if(totalWidth > viewportW && !options.forceFitColumns)
					$viewport.height(newViewportH + scrollbarDimensions.height);
				else
			...
			if (self.onResizeCanvas) {
				self.onResizeCanvas({viewport:$viewport, container:$container, width:totalWidth, height:newViewportH});
			}
            updateRowCount();
            render();
        }
        ...
        function resizeAndRender() {
            ...
            var windowNewOffset = $(window).width() - ($container.parent().offset().left + $container.parent().width());
            windowNewOffset = (windowNewOffset >= windowInitialOffset ? windowInitialOffset : windowNewOffset);
            //$container.css('max-width', $(window).width() - $container.parent().offset().left - windowNewOffset);
        ...
        function updateRowCount() {
			...
			else if (options.rowHeight * newRowCount < canvasHeight && options.autoHeight) {
				resizeCanvas();
			}
        }
            ...
            if (!handled) {
                if (!e.shiftKey && !e.altKey && !e.ctrlKey) {
                    if (e.which == 27) {
						...
						setTimeout(makeSelectedCellEditable, options.asyncEditorLoadDelay);
                        navigateNext();
                    }
        ...
        function handleClick(e) {
        	if (e.ctrlKey && e.target.tagName.toLowerCase() == "a") {
        		return true;
        	}
			...
			// editor is open and we didn't click in the cell currently editing
			if (currentEditor !== null) {
				if(currentEditor.handleClick)
					currentEditor.handleClick(e, row, cell);
			}
            ...
            if (options.enableCellNavigation && !columns[cell].unselectable) {
                ...
                if (validated === true || (validated === null && options.editorLock.commitCurrentEdit())) {
                    ...
                    setSelectedCellAndRow($cell[0], (row === defaultGetLength()) || options.autoEdit || isCellPotentiallyEditable(row,cell) );
                }
            }
        }
		...
		function getCellFromCoords(row, cell) {
			return $($(rowsCache[row]).children().eq(cell));
		}
        ...
        function setSelectedCell(newCell,editMode) {
            if (currentCellNode !== null) {
				...
				$(currentCellNode).removeAttr("hidefocus");
				$(currentCellNode).removeAttr("tabindex");
            }
