...
abstract class MongoForm extends PageListForm {
	...
	const ID_TYPE_AUTO_INC = 2;
	const ID_TYPE_STRING = 3;
	...
	public function insert() {
		...
		if ($this->getIdType() === self::ID_TYPE_AUTO_INC) {
			...
			$counter_array = $this->getCollection('default', 'counter')->findAndModify(
				array('_id' => $counter_name),
				array('$inc' => array('counter' => 1)),
				null,
				array('new' => true, 'upsert' => true)
			);
			$new_id = $counter_array['counter'];
			$this->setId($new_id);
		...
		*/
			...
			$ret_val = $this->getCollection()->save($insert_array);
			if (array_key_exists('_id', $insert_array)) {
		...
		foreach ($this->toArray(true, true, true) AS $name => $value) {
			if (in_array($name, $this->getModifiedColumns())) {
				...
				if(strpos($name, '_') === 0) {
					...
					if (is_array($value) && ($use_set_notation === true)) {
						$set_notation_array = $this->createSetNotation($value);
						foreach($set_notation_array AS $set_node_id => $set_node_value) {
							$set_data_array[$name . '.' . $set_node_id] = $set_node_value;
						}
					} else {
						$set_data_array[$name] = $value;
					}
				}
			}
		}
